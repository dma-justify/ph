...
namespace Bitsplash.DatePicker
{
    ...
    public class StandardDatePickerCell : DatePickerCell
    {
        ...
        [NonSerialized] public Color MarkSelectedColor = new Color(0f, 0f, 0f, 0f);
        ...
        void AssignTextAndBackground(bool dontSlide = false)
        {
            ...
            if (dataStorage != null && dataStorage.Habits != null)
            {
                var habits = dataStorage.Habits.Where(habit =>
                {
                    if (mDayValue.DayOfYear == habit.CreateDate.DayOfYear && habit.CreateDate.Year == mDayValue.Year)
                        return true;
                    TimeSpan dif = habit.CreateDate - mDayValue;
                    return (mDayValue >= habit.CreateDate && dif.Days % (habit.Interval + 1) == 0);
                }).ToArray();
            
            
                positive.SetActive(habits.Any(h => h.Positive));
                negative.SetActive(habits.Any(h => !h.Positive));
            }
            else
            {
